...
class AppLauncher(Box):
    ...
    def on_search_entry_activate(self, text):
        # This method is now only used for non-Enter key activations
        ...
        if text in commands:
            ...
            eval(command, {"launcher": self.launcher})
        else:
    ...
    def notify_text(self, entry, *_):
        ...
        text = entry.get_text()
        if text.startswith("="):
            # In calculator mode, show scrolled window and update viewport
            self.show_scrolled_window()
            self.launcher.dashboard.hide()
            self.update_calculator_viewport()
    ...
    def add_selected_app_to_dock(self):
        """Adds the currently selected application to the modus.json file with comprehensive metadata."""
        children = self.viewport.get_children()
        if (
            not children
