...
class InlineEditor(Gtk.Box):
    ...
    def __init__(self, initial_text=""):
        ...
        confirm_btn = Gtk.Button(
            name="kanban-btn", child=Label(name="kanban-btn-label", markup=icons.accept)
        )
        confirm_btn.connect("clicked", self.on_confirm)
        confirm_btn.get_style_context().add_class("flat")

        cancel_btn = Gtk.Button(
            name="kanban-btn", child=Label(name="kanban-btn-neg", markup=icons.cancel)
        )
...
class Kanban(Gtk.Box):
    STATE_FILE = Path(os.path.expanduser("~/.local/state/modus/kanban.json"))
