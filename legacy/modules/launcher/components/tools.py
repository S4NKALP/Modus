...
CAPTURE_SCRIPT = get_relative_path("../../../config/scripts/capture.sh")
GAMEMODE_SCRIPT = get_relative_path("../../../config/scripts/gamemode.sh")
...
class Toolbox(Box):
    ...
    def ssfull(self, *args):
        exec_shell_command_async(f"bash {CAPTURE_SCRIPT} screenshot p")
        self.close_menu()
        ...
        if event.type == Gdk.EventType.BUTTON_PRESS:
            if event.button == 1:  # Left click
                ...
                exec_shell_command_async(
                    f"bash {CAPTURE_SCRIPT} screenshot p mockup")
                self.close_menu()
            return True
        return False
    ...
    def ssregion(self, *args):
        exec_shell_command_async(f"bash {CAPTURE_SCRIPT} screenshot sf")
        self.close_menu()
    ...
    def on_ssregion_click(self, button, event):
        if event.type == Gdk.EventType.BUTTON_PRESS:
            if event.button == 1:  # Left click
                ...
                exec_shell_command_async(
                    f"bash {CAPTURE_SCRIPT} screenshot sf mockup")
                self.close_menu()
            return True
        return False
    ...
    def screenrecord(self, *args):
        ...
        exec_shell_command_async(
            f"bash -c 'nohup bash {CAPTURE_SCRIPT} record > /dev/null 2>&1 & disown'"
        )
        self.close_menu()

    def ocr(self, *args):
        exec_shell_command_async(f"bash {CAPTURE_SCRIPT} ocr sf")
        self.close_menu()
    ...
    def colorpicker_key(self, widget, event):
        if event.keyval in {Gdk.KEY_Return, Gdk.KEY_KP_Enter}:
            ...
            exec_shell_command_async(
                f"bash {get_relative_path('../../../config/scripts/hyprpicker.sh')} {
                    cmd
                }"
            )
            self.close_menu()
    ...
    def open_screenshots_folder(self, *args):
        screenshots_dir = os.path.join(
            os.environ.get("XDG_PICTURES_DIR",
