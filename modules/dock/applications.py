...
class Applications(Box):
    ...
    def on_button_press(self, widget, event):
        ...
        if event.button == 3:  # Right mouse button
            ...
            for pinned_app in self.pinned:
                if (isinstance(app_identifier, dict) and isinstance(pinned_app, dict) and 
                    app_identifier.get("name") == pinned_app.get("name")):
                    is_pinned = True
                    break
    ...
    def on_drag_begin(self, widget, drag_context):
        ...
        Gtk.drag_set_icon_surface(drag_context, self.create_surface_from_widget(widget))

    def create_surface_from_widget(self, widget):
        # Create a surface from the widget for drag icon
    ...
    def on_drag_data_received(self, widget, drag_context, x, y, data_obj, info, time):
        ...
        if source_index != target_index:
            # Reorder the pinned apps
            if source_index < len(self.pinned) and target_index < len(self.pinned):
    ...
    def on_drag_end(self, widget, drag_context):
        if not hasattr(self, "_drag_in_progress") or not self._drag_in_progress:
            return
        ...
        if widget_x is None:  # Widget might not be in the same window
            widget_x, widget_y = 0, 0
        widget_width, widget_height = widget_alloc.width, widget_alloc.height
        ...
        if not (widget_x <= x <= widget_x + widget_width and 
            ...
            # Remove pinned app if it's in the pinned list
            app_index_dragged = -1
