...
# FIX: Make sure the network only scan when the pull to refresh started
# TODO: Show visual Feeadback and animation like bluetooth
...
class WifiConnections(Box):
    ...
    def on_network_ready(self, *_):
        ...
        if self.wifi_service:
            ...
            self.wifi_service.connect("scanning", self.on_scanning_changed)
    ...
    def on_scanning_changed(self, service, is_scanning):
        """Handle scanning state changes"""
        print(f"WiFi scanning state changed: {is_scanning}")  # Debug output
        self.update_scan_indicator(is_scanning)
    ...
    def on_scroll_event(self, widget, event):
        ...
        if self.vadjustment.get_value() <= 0:
            if event.direction == Gdk.ScrollDirection.UP:
                ...
                print("WiFi scroll scan triggered")  # Debug output
                if self.wifi_service:
    ...
    def on_button_release(self, widget, event):
        ...
        if self.is_pulling:
            ...
            if pull_distance > self.pull_threshold:
                # Trigger scan
                print("WiFi pull-to-refresh scan triggered")  # Debug output
                if self.wifi_service:
                    self.wifi_service.scan()
    ...
    def update_scan_indicator(self, is_scanning):
        """Update scanning state appearance"""
        print(f"WiFi update_scan_indicator called: {is_scanning}")  # Debug output
        if is_scanning:
            ...
            print("WiFi scanning indicator shown")  # Debug output
        else:
            # Hide scanning feedback
            if not self.is_pulling:
                self.refresh_indicator.set_visible(False)
            self.refresh_indicator.remove_style_class("scanning")
            print("WiFi scanning indicator hidden")  # Debug output
    ...
    def start_bounce_animation(self):
        ...
        print("WiFi bounce animation started")  # Debug output
    ...
    def start_elastic_animation(self):
        ...
        print("WiFi elastic animation started")  # Debug output
        if self.bounce_timeout_id:
            GLib.source_remove(self.bounce_timeout_id)
