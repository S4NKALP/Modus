...
class PomodoroTimer:
    ...
    def _send_notification(self, title, message, icon="🍅"):
    ...
    def _send_countdown_notification(self, seconds_left):
            ...
            for countdown in [3, 2, 1]:
                ...
                if delay > 0:
                    timer = threading.Timer(
                        delay, self._send_countdown_notification, [countdown]
                    )
                    timer.start()
    ...
    def get_status(self):
        ...
        remaining_seconds = max(0, int(self.end_time - time.time()))
        ...
        return f"{period_type} - {remaining_minutes:02d}:{remaining_seconds:02d} | Total: {self.total_cycles_completed}"
        ...
        if self.pomodoro.is_active():
            ...
            results.append(
                Result(
                    ...
                    action=lambda: self.pomodoro.stop(),
                    relevance=0.9,
                    plugin_name=self.display_name,
                    data={"type": "stop"},
                )
            ...
            if not query_string.strip():
                ...
                results.append(
                    Result(
                        ...
                        icon_markup=icons.timer_on,
                        action=lambda: self.pomodoro.start(
                ...
                if len(tokens) > 4 or not all(t.isdigit() for t in tokens):
                    ...
                    results.append(
                        Result(
                            ...
                            icon_markup=icons.timer_on,
                            action=lambda _p=p, _b=b, _lb=lb, _c=c: self.pomodoro.start(
                                _p, _b, _lb, _c
                            ),
                            relevance=1.0,
                            plugin_name=self.display_name,
