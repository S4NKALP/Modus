...
class TriggerConfig:
    ...
    def get_trigger_examples(self, trigger: str) -> List[str]:
        ...
        if not examples:
            template = self.settings.get('fallback_example_template', '{trigger} <search>')
            examples = [template.format(trigger=trigger)]
        return examples
    
    def get_trigger_icon(self, trigger: str) -> str:
        icon_name = self.launcher_config.get(trigger, {}).get('icon', self.settings.get('default_icon', 'apps'))
        return getattr(icons, icon_name, icons.apps)
    
    def get_trigger_description(self, trigger: str) -> str:
        return self.launcher_config.get(trigger, {}).get('description', f'{trigger} - No description available')
    
    def get_all_triggers(self) -> Dict[str, Dict[str, Any]]:
        return self.launcher_config.copy()
    
    def add_trigger(self, trigger: str, examples: List[str], icon: str = None, description: str = None) -> bool:
        try:
