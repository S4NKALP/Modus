...
class WallpaperSelector(Box):
    ...
    def __init__(self, **kwargs):
        super().__init__(name="wallpapers", spacing=5, orientation="v", **kwargs)
        ...
        self.executor = ThreadPoolExecutor(max_workers=5)  # Shared executor

        self.viewport = Gtk.IconView()
        self.viewport.set_model(Gtk.ListStore(GdkPixbuf.Pixbuf, str))
        self.viewport.set_pixbuf_column(1)
        self.viewport.set_text_column(2)
        self.viewport.set_item_width(1)
        ...
        self.custom_color_entry = Entry(
            name="custom-color-entry",
            h_expand=True,
            # notify_text=self.on_custom_color_changed,
        )
        self.custom_color_entry.connect("activate", self.on_custom_color_submitted)

        self.color_preview = Box(
            name="color-preview",
        )
        self.custom_color_box = Box(
            ...
            orientation="h",
            children=[
                Label(label="Custom Color:", xalign=0),
    ...
    def arrange_viewport(self, query: str = ""):
        ...
        filtered_thumbnails.sort(key=lambda x: x[2].lower())
    ...
    # def on_custom_color_changed(self, entry: Entry, *_):
    #     """Live preview for color entry changes."""
    #     color = entry.get_text().strip()
    #     if is_valid_hex_color(color):
    #         self.color_preview.override_background_color(
    #             Gtk.StateFlags.NORMAL, Gdk.RGBA.from_string(color)
    #         )
    #     elif is_valid_hue(color):
    #         hex_color = self.hue_to_hex(float(color))
    #         self.color_preview.override_background_color(
    #             Gtk.StateFlags.NORMAL, Gdk.RGBA.from_string(hex_color)
    #         )
    #     else:
    #         self.color_preview.override_background_color(Gtk.StateFlags.NORMAL, None)
