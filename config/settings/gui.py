...
class HyprConfGUI(Window):
    ...
    def create_appearance_tab(self):
        ...
        dock_auto_hide_label = Label(label="Auto-hide Dock", h_align="start", v_align="center")
        layout_grid.attach(dock_auto_hide_label, 0, 2, 1, 1)
        dock_auto_hide_switch_container = Gtk.Box(orientation=Gtk.Orientation.HORIZONTAL, halign=Gtk.Align.START, valign=Gtk.Align.CENTER)
        self.dock_auto_hide_switch = Gtk.Switch(active=bind_vars.get('dock_auto_hide', False), sensitive=self.dock_switch.get_active())
    ...
    def create_system_tab(self):
        ...
        metric_names = {'cpu': "CPU", 'ram': "RAM", 'disk': "Disk", 'swap': "Swap", 'gpu': "GPU"}
    ...
    def create_about_tab(self):
        ...
        repo_link = Label(markup='<a href="https://github.com/S4NKALP/Modus">https://github.com/S4NKALP/Modus</a>')
        repo_box.add(repo_label); repo_box.add(repo_link)
        vbox.add(repo_box)
        def on_kofi_clicked(_): import webbrowser; webbrowser.open("https://ko-fi.com/S4NKALP")
        kofi_btn = Button(label="Support on Ko-Fi ❤️", on_clicked=on_kofi_clicked, tooltip_text="Support S4NKALP on Ko-Fi", style="margin-top: 18px; min-width: 160px;")
        vbox.add(kofi_btn)
    ...
    def on_accept(self, widget):
        ...
        current_bind_vars_snapshot['dock_position'] = self.position_combo.get_active_text()
        current_bind_vars_snapshot['vertical'] = current_bind_vars_snapshot['dock_position'] in ["Left", "Right"]
        
        current_bind_vars_snapshot['dock_enabled'] = self.dock_switch.get_active()
        current_bind_vars_snapshot['dock_auto_hide'] = self.dock_auto_hide_switch.get_active()
    ...
    def on_reset(self, widget):
        ...
        if response == Gtk.ResponseType.YES:
            ...
            self.dock_auto_hide_switch.set_active(utils.bind_vars.get('dock_auto_hide', False))
            self.dock_hover_switch.set_active(utils.bind_vars.get('dock_always_occluded', False))
            ...
            for name, switch in self.component_switches.items():
                 switch.set_active(utils.bind_vars.get(f'bar_{name}_visible', True))
