...
class ScreenCapture(Service):
    ...
    def screenshot(self, fullscreen=False, save_copy=True):
        ...
        # hyprshot: --mode [window|output|region], --stdout, --clipboard, --save-path
        if save_copy:
            command = [
                "hyprshot",
                "--mode",
                "output" if fullscreen else "region",
                "--clipboard",
                "--save-path",
                file_path,
            ]
        ...
        try:
            exec_shell_command_async(
                ...
                lambda *_: self.send_screenshot_notification(
                    file_path=file_path if save_copy else None,
                ),
            )
    ...
    def screencast_start(self, fullscreen=False, audio=False):
        ...
        audio_flag = "--audio --no-mic" if audio else ""
        command = f"wf-recorder --file={file_path} --pixel-format yuv420p {audio_flag} {area}"
